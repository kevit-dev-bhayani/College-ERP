...
class StaffController {
    ...
    async findStaff(req, res, next) {
        try {
            const staff = await (0, staff_DAL_1.findStaffById)(req.params.id);
            if (staff) {
                return res.status(200).send({ data: staff });
            }
        ...
        try {
            ...
            if (staff) {
                ...
                if (match) {
                    ...
                    const token = jwt.sign({ id: staff.id, phone_no: staff.phone_no, department_id: staff.department_init }, privateKey, { algorithm: 'RS256' });
