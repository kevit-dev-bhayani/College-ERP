...
class StudentsController {
    async createStudents(req, res, next) {
        try {
            ...
            const student = await (0, staff_DAL_1.createNewStaff)(studentObject);
    ...
    async getStudents(req, res, next) {
        try {
            const students = await (0, staff_DAL_1.findStaff)();
    ...
    async findStudent(req, res, next) {
        try {
            const student = await (0, staff_DAL_1.findStaffById)(req.params.id);
            if (student) {
                return res.status(200).send({ data: student });
            }
    ...
    async deleteStudent(req, res, next) {
        try {
            ...
            await (0, staff_DAL_1.deleteStaffById)(req.params.id);
            return res.status(200).send({ data: student });
        }
        catch (err) {
            return next(err);
    ...
    async loginStudent(req, res, next) {
        try {
            ...
            const student = await (0, staff_DAL_1.findStaffByPhoneNo)(phone_no);
            if (student) {
                const match = await bcrypt.compare(password, student.password);
                if (match) {
                    const privateKey = fs.readFileSync((0, path_1.join)(__dirname, '../../../keys/Private.key'));
                    const token = jwt.sign({ id: student.id, phone_no: student.phone_no, department_id: student.department_init }, privateKey, { algorithm: 'RS256' });
...
//# sourceMappingURL=staff.contoller.js.map