...
import { findDepartmentById,findAllDepartments,CreateNewDepartment,findDepartmentByInitialize ,deleteDepartmentById } from './department.DAL';
...
class DepartmentController{
    ...
    async updateDepartment(req,res,next){
        try {
            ...
            const department=await findDepartmentById(req.params.department_id);
    ...
    async FindByInit(req,res,next){
        try {
            const department=await findDepartmentByInitialize(req.params.init);
            return res.status(200).send({ data:department });
        } catch (err) {
            return next(err);
        }
